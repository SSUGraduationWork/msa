...
public class BoardService {
    ...
    public List<MemberDto> teamMemberList(Long teamId) {
    ...
    public BoardDetailResponse boardView(Long id){
        ...
        MemberDto memberDto =memberServiceClient.findByUserId(boards.getUserId());
    ...
    public void reWrtieCompletionAlarm(Boards boards){
        ...
        MemberDto writers=memberServiceClient.findByUserId(boards.getUserId());
        WorkResponse workResponse=workerServiceClient.findWorkById(boards.getWorkId());
        for (int i = 0; i < allMembers.size(); i++) {
            MemberDto memberDto =allMembers.get(i);
            FeedbackStatuses feedbackStatuses=feedbackStatusesList.get(i);
            if (memberDto == null || memberDto.getId().equals(boards.getUserId())) {continue;}

                //만약 글 작성자 본인이라면 피드백 승인,거부를 할 필요가 없으므로 feedbackStatus 등록 필요 없음.
                //알림 기능, 글 등록 시 모든 팀의 모든 팀원들에게 알람이 감

            String userName = memberDto.getName(); //글 작성자 이름
            ...
            alarms.setUserId(memberDto.getId()); // 연관관계 설정
    ...
    public void FeedbackStatusAndAlarm(Boards boards, Long writerId, Long workId, Long teamId) {
        ...
        List<MemberDto> allMembers = teamServiceClient.findTeamById(boards.getTeamId());
        MemberDto writers=memberServiceClient.findByUserId(writerId);
